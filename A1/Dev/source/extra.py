...
import more_test_problems
from more_test_problems import ADDITIONAL_PROBLEMS as PROBLEMS
...
if __name__ == '__main__':
  if test_time_astar:
    ...
    time_bound = 3
    p = multiprocessing.Process(target=anytime_weighted_astar, name="Anytime A star", args=(PROBLEMS[9],heur_alternate,10,time_bound))
    p.start()
  ...
  if test_time_gbfs:
    ...
    time_bound = 3
    q = multiprocessing.Process(target=anytime_gbfs, name="Anytime GBFS", args=(PROBLEMS[9],heur_alternate,time_bound))
    q.start()
  ...
  if test_anytime_weighted_astar:
    ...
    for i in range(0, len(PROBLEMS)):
      ...
      # But this!
      final = anytime_weighted_astar(s0, heur_fn=heur_alternate, timebound=timebound)
